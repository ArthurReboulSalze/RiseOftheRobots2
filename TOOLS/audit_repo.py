...
def main():
    ...
    parser.add_argument("--staged", action="store_true", help="L'index complet est toujours contrôlé (alias explicite)")
    parser.parse_args()
    ...
    for entry in entries:
        ...
        if mode != "100644" and mode != "100755" or stage != "0" or not allowed(path):
            errors.append(f"Chemin/type non autorisé : {path}")
            continue
        blob = subprocess.check_output(["git", "cat-file", "blob", oid])
        total += len(blob)
        if len(blob) > 1024 * 1024 or b"\0" in blob:
            errors.append(f"Fichier binaire ou trop gros : {path}")
            continue
        try:
            text = blob.decode("utf-8-sig")
        except UnicodeDecodeError:
            errors.append(f"Texte non UTF-8 : {path}")
            continue
        ...
        if any(re.search(pattern, text) for pattern in patterns):
            errors.append(f"Secret ou média embarqué potentiel : {path}")
    if not count:
        errors.append("Index vide : aucune liste de fichiers à contrôler.")
    for error in errors:
        print(error, file=sys.stderr)
    print(f"Audit index : {count} fichiers, {total:,} octets de texte, {len(errors)} erreur(s).")
    return bool(errors)
